...
class DeepSeek():
    def __init__(self, conf: dict) -> None:
        ...
        self.LOG = logging.getLogger("DeepSeek openAI")
        timeout_config = httpx.Timeout(30.0)
        if proxy:
            self.client = OpenAI(api_key=key, base_url=api, http_client=httpx.Client(proxy=proxy), timeout=timeout_config)
        else:
    ...
    def get_answer(self, question: str, wxid: str) -> str:
        # 检查最后一条消息是否为助手的消息，若不是则等待直至获得响应。
        if wxid in self.conversation_list and len(self.conversation_list[wxid]) > 0 and \
                self.conversation_list[wxid][-1]["role"] != "assistant":
            # 这里假设我们不实际等待，而是依赖于串行调用来自然地解决这个问题。
            # 实际应用中可能需要更复杂的逻辑来处理并发场景。
            self.updateMessage(wxid, "提问超时", "assistant")
